...
public Action:Timer_Enemies_Remaining(Handle:Timer)
{
	...
	if (validAntenna != -1 || g_jammerRequired == 0)
	{
		...
		PrintToChatAll(textToPrintChat);
		new jammerSpawnReductionAmt = (g_iRespawnSeconds / (GetTeamSecCount() / 3));
		Format(textToPrintChat, sizeof(textToPrintChat), "干扰器已激活！ | 复活时间将缩短: %d 秒", jammerSpawnReductionAmt);
		PrintToChatAll(textToPrintChat);
...
public Action:Event_PlayerPickSquad_Post( Handle:event, const String:name[], bool:dontBroadcast )
{
	...
	else if (StrContains(g_client_last_classstring[client], "spotter") > -1)
	{
		// Admin medic
		if (GetConVarInt(sm_respawn_enable_donor_tag) == 1 && (GetUserFlagBits(client) & ADMFLAG_ROOT))
			Format(sNewNickname, sizeof(sNewNickname), "[管理][神射] %s", g_client_org_nickname[client]);
		// Donor medic
		else if (GetConVarInt(sm_respawn_enable_donor_tag) == 1 && (GetUserFlagBits(client) & ADMFLAG_RESERVATION))
			Format(sNewNickname, sizeof(sNewNickname), "[会员][神射] %s", g_client_org_nickname[client]);
		// Normal medic
		else
	...
	else if (StrContains(g_client_last_classstring[client], "pointman") > -1)
	{
		// Admin medic
		if (GetConVarInt(sm_respawn_enable_donor_tag) == 1 && (GetUserFlagBits(client) & ADMFLAG_ROOT))
			Format(sNewNickname, sizeof(sNewNickname), "[管理][步枪(定点)] %s", g_client_org_nickname[client]);
		// Donor medic
		else if (GetConVarInt(sm_respawn_enable_donor_tag) == 1 && (GetUserFlagBits(client) & ADMFLAG_RESERVATION))
			Format(sNewNickname, sizeof(sNewNickname), "[会员][步枪(定点)] %s", g_client_org_nickname[client]);
		// Normal medic
		else
	...
	else if (StrContains(g_client_last_classstring[client], "support") > -1)
	{
		// Admin medic
		if (GetConVarInt(sm_respawn_enable_donor_tag) == 1 && (GetUserFlagBits(client) & ADMFLAG_ROOT))
			Format(sNewNickname, sizeof(sNewNickname), "[管理][机枪] %s", g_client_org_nickname[client]);
		// Donor medic
		else if (GetConVarInt(sm_respawn_enable_donor_tag) == 1 && (GetUserFlagBits(client) & ADMFLAG_RESERVATION))
			Format(sNewNickname, sizeof(sNewNickname), "[会员][机枪] %s", g_client_org_nickname[client]);
		// Normal medic
		else
	...
	else if (StrContains(g_client_last_classstring[client], "rifleman_at") > -1)
	{
		// Admin medic
		if (GetConVarInt(sm_respawn_enable_donor_tag) == 1 && (GetUserFlagBits(client) & ADMFLAG_ROOT))
			Format(sNewNickname, sizeof(sNewNickname), "[管理][步枪(AT)] %s", g_client_org_nickname[client]);
		// Donor medic
		else if (GetConVarInt(sm_respawn_enable_donor_tag) == 1 && (GetUserFlagBits(client) & ADMFLAG_RESERVATION))
			Format(sNewNickname, sizeof(sNewNickname), "[会员][步枪(AT)] %s", g_client_org_nickname[client]);
		// Normal medic
		else
	...
	else if (StrContains(g_client_last_classstring[client], "vip") > -1)
	{
		// Admin medic
		if (GetConVarInt(sm_respawn_enable_donor_tag) == 1 && (GetUserFlagBits(client) & ADMFLAG_ROOT))
			Format(sNewNickname, sizeof(sNewNickname), "[管理][VIP] %s", g_client_org_nickname[client]);
		// Donor medic
		else if (GetConVarInt(sm_respawn_enable_donor_tag) == 1 && (GetUserFlagBits(client) & ADMFLAG_RESERVATION))
			Format(sNewNickname, sizeof(sNewNickname), "[会员][VIP] %s", g_client_org_nickname[client]);
		// Normal medic
		else

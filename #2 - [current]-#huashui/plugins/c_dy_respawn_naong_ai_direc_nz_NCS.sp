...
#define SERVER_VERSION "anilla"
#define GAMEDESC "女装娱乐服"
...
{
	Format(gameDesc, sizeof(gameDesc), "%s v%s", GAMEDESC, SERVER_VERSION);
...
void UpdateRespawnCvars()
{
	...
	if (g_easterEggRound == true)
	{
		...
		ConVar cvar_mp_maxrounds = FindConVar("mp_maxrounds");
		SetConVarInt(cvar_mp_maxrounds, 2, true, true);
		ConVar cvar_sm_botspawns_min_player_distance = FindConVar("sm_botspawns_min_player_distance");
...
public Action:Timer_WaveSpawning(Handle:Timer)
{
	...
	if (g_respawn_mode_team_sec)
	{
			...
			g_secWave_Timer <= 3)
		{
			decl String:textToPrintChat[64];
			//decl String:textToPrint[64];
			Format(textToPrintChat, sizeof(textToPrintChat), "将在 %d 秒后增援", g_secWave_Timer);
			//Format(textToPrint, sizeof(textToPrint), "将在 %d 秒后团队增援", g_secWave_Timer);
			//PrintHintTextToAll(textToPrint);
			PrintToChatAll(textToPrintChat);
		}
		...
		{
			PrintHintTextToAll("增援已到达！");
			PrintToChatAll("增援已到达！");
...
public Action:Timer_Enemies_Remaining(Handle:Timer)
{
	...
	else
	{
		...
		PrintHintTextToAll(textToPrint);
		PrintToChatAll(textToPrintChat);
		Format(textToPrintChat, sizeof(textToPrintChat), "干扰器已激活！ | 复活时间将缩短： %d 秒", 0);
		PrintToChatAll(textToPrintChat);
	}
	return Plugin_Continue;
}
...
void AI_Director_RandomEnemyReinforce()
{
	...
	if (g_iRemaining_lives_team_ins > 0)
	{
		...
		// {
			...
			CreateTimer(45.0, Timer_ResetBotFlankSpawning);
		//}
	}
	else
	{
...
#####################################################################
*/
stock Float:GetInsSpawnGround(spawnPoint, Float:vecSpawn[3])
{
	new Float:fGround[3];
	vecSpawn[2] += 15.0;
...
CheckSpawnPoint(float vecSpawn[3], int client, float tObjectiveDistance, int m_nActivePushPointIndex) {
//Ins_InCounterAttack
	new m_iTeam = GetClientTeam(client);
...
float GetSpawnPoint_SpawnPoint(client) {
	...
	float distance;
	GetClientAbsOrigin(client,vecOrigin);
	new Float:fRandomFloat = GetRandomFloat(0.0, 1.0);
	...
	while (point != -1) {
		...
		Ins_ObjectiveResource_GetPropVector("m_vCPPositions",m_vCPPositions[m_nActivePushPointIndex],m_nActivePushPointIndex);
		distance = GetVectorDistance(vecSpawn,m_vCPPositions[m_nActivePushPointIndex]);
		if (CheckSpawnPoint(vecSpawn,client,tObjectiveDistance,m_nActivePushPointIndex) == 1) {
...
public Action:Event_RoundStart(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	ConVar cvar_scoreboard = FindConVar("sv_hud_scoreboard_show_score_dead");
...
public Action:Event_ControlPointCaptured_Pre(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (fRandom < g_respawn_counter_chance && g_isCheckpoint == 1 && ((acp+1) != ncp))
	{
		...
		g_isEliteCounter = 1;
		CreateTimer(view_as<float>(cvar_ca_dur.IntValue + 1.0), Timer_CounterAttackEnd);
	...
	else if (g_isCheckpoint == 1 && ((acp+1) == ncp))
	{
		...
		//Create Counter End Timer
		g_isEliteCounter = 1;
		CreateTimer(view_as<float>(cvar_ca_dur.IntValue + 1), Timer_CounterAttackEnd);
					...
					for(new client = 1; client <= MaxClients; client++)
					{
						...
						if((IsValidClient(client)) && (IsClientConnected(client)) && (!IsFakeClient(client)))
						{
							...
							if(nSupplyPoint <= nMaxSupply)
							{
								...
								PrintToChat(client, "\x04VIP\x01 已占领检查点\n你被奖励了 \x04%i\x01 点补给点", nRandSupplyReward);
								PrintHintText(client,"VIP占领了检查点\n奖励 %i 点补给点", nRandSupplyReward);
							}
...
void EnableDisableEliteBotCvars(tEnabled, isFinale)
{
	...
	if (tEnabled == 1)
	{
		...
		tCvarIntValue = tCvarIntValue - view_as<int>(g_bot_attack_aimtolerance_newthreat_amt_mult);
		SetConVarFloat(tCvar, tCvarFloatValue, true, false);
			...
			)
			{
				...
				)
				{
					...
					{	
						...
						// Hint to iMedic
						Format(sBuf, 255,"你从 %s 中救起了 \x04%N\x01 | 在奖励复活次数前你还要救起: \x04%d\x01 人", woundType, iInjured, (iReviveCap - g_playerMedicRevivessAccumulated[iMedic]));
						PrintHintText(iMedic, "%s", sBuf);
		...
		else if (iTeam == TEAM_1_SEC && IsPlayerAlive(medic) && !(StrContains(g_client_last_classstring[medic], "medic") > -1))
		{
			...
			if ((StrContains(checkWeapon, "weapon_healthkit") > -1))
			{
				...
				if(iTarget > 0 && iTarget <= MaxClients && IsClientInGame(iTarget) && IsPlayerAlive(iTarget) && iTeam == GetClientTeam(iTarget))
				{
					...
					{
						// Check weapon
						ActiveWeapon = GetEntPropEnt(medic, Prop_Data, "m_hActiveWeapon");
						if (ActiveWeapon < 0)
							continue;
...
public Action:Timer_VIPCheck_Main(Handle:timer, any:data)
{
	...
	//decl String:textToPrint[64];
	// Announce every 10 seconds
	if (g_vip_obj_count > 0 && (g_iCvar_vip_obj_time - g_vip_obj_count) % 10 == 0)
	{				
		...
		if (g_nVIP_ID == 0)
		{
			Format(textToPrintChat, sizeof(textToPrintChat), "当前队伍中没有 \x04VIP\x01 | 占领检查点将不会获得额外的补给点。");
			//PrintHintTextToAll(textToPrint);
			PrintToChatAll(textToPrintChat);
			//PrintCenterText(textToPrintChat);
		}
	}
	if (g_vip_obj_count == 0 && g_vip_obj_ready != 0)
	{
		...
		{
			g_vip_obj_ready = 0;
			PrintHintTextToAll("\x04VIP 没有在 %d 秒内占领检查点，附加任务失败。", g_iCvar_vip_obj_time);
			PrintToChatAll("\x04VIP 没有在 %d 秒内占领检查点，附加任务失败。", g_iCvar_vip_obj_time);
		}
	}
		...
		if (IsClientConnected(friendlyMedic) && IsClientInGame(friendlyMedic) && !IsFakeClient(friendlyMedic))
		{
			...
			if (IsPlayerAlive(friendlyMedic) && (StrContains(g_client_last_classstring[friendlyMedic], "medic") > -1) && client != friendlyMedic)
			{
				...
				if (fDistance <= fReviveDistance && bCanHealPaddle)
				{
					...
					Format(sBuf, 255,"你从 %s 中协助救起了 \x04%N\x01 | 在奖励复活次数前你还要救起: \x04%d\x01 人", woundType, iInjured, (iReviveCap - g_playerMedicRevivessAccumulated[friendlyMedic]));
...
public AI_Director_SetDifficulty() 
{
	...
	new 
		...
		Float:AID_SpecDelayAdj_low = 10.0, Float:AID_SpecDelayAdj_med = 20.0, Float:AID_SpecDelayAdj_high = 30.0,Float:AID_SpecDelayAdj_pScale_Pro = 0.0, Float:AID_SpecDelayAdj_pScale_Con = 0.0,
...
public Action OnTakeDamageHook(int victim, int &attacker, int &inflictor, float &damage, int &damagetype)
{
	...
	if (!g_iPlayerBleeding[victim])
	{
		if (team == TEAM_1_SEC)
		{
			...
			{
				if (GetRandomFloat(0.1, 100.00) <= 0.0)
					bBleeding = true;
			}

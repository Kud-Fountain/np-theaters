...
new Float:FCVAR_ZOMBIE_STALKER_SIZE_MAX					=		0.77;
...
new g_iMedicPlayer = -1;
new Float:g_fMedicLastHealTime = 0.0;
new Float:g_fMedicBannedTime[MAXPLAYERS_INS+1] = {0.0, ...};
new bool:g_bMedicForceToChange = false;
...
//	ClientCommand(client, "play ui/sfx/noise_04.wav");
	...
	if (iGear == 8 && g_iMedicPlayer == client)
	{
...
//	if (type < 0 || type > 4) type = 2;
	...
	if (g_iCustomFlagIndex[client] != INVALID_ENT_REFERENCE && IsValidEntity(g_iCustomFlagIndex[client]))
	{
		...
		if (iFlag > MaxClients)
		{
			SetEntProp(iFlag, Prop_Data, "m_nSkin", g_iMedicPlayer!=client?2:3);
			if (g_iMedicPlayer == client) SetEntProp(client, Prop_Send, "m_bGlowEnabled", 1);
			else SetEntProp(client, Prop_Send, "m_bGlowEnabled", 0);
			LogToGame("Custom Flag for \"%N\" has been already set and use it instead doing recreate (Skin: %s, Index: %d (%d))", client, g_iMedicPlayer!=client?"USA":"MEDIC", iFlag, g_iCustomFlagIndex[client]);
	...
	if (iFlag != -1 && IsValidEntity(iFlag))
	{
		...
		LogToGame("Custom Flag for \"%N\" Has created (Skin: %s, Index: %d (%d))", client, g_iMedicPlayer!=client?"USA":"MEDIC", iFlag, g_iCustomFlagIndex[client]);
...
//									PrintCenterText(client, " ");
	...
	if (iWorkStatus == 0 && g_iPlayerDeployedWeapon[client] == WEAPON_HEALTHKIT)	// #Deployed weapon_healthkit (40)
	{
		...
		if (Healthkit_HasHolding(client))
		{
			...
			new iTarget;
			new bool:bMedic = g_iMedicPlayer==client?true:false;
			new iButtons = GetClientButtons(client);
...
//								EmitSoundToAll("Lua_sounds/bandaging.wav", client, SNDCHAN_STATIC, _, _, 0.8);
	...
	{
		if (g_iMedicPlayer != client && GetClientButtons(client) & INS_USE)
		{
...
stock UpdateBotsConfig(iPlayers = 0, bool:bNotice = true, bool:bOnlyNoticeWhenUpdated = false)
{
	...
	switch(iPlayers)
	{
//		case 11, 12: iBotCount = 21;
/*		case 9, 10: iBotCount = 21;	*/
		case 7, 8: iBotCount = 22;
		case 5, 6: iBotCount = 18;
		case 4: iBotCount = 15;
		case 3: iBotCount = 12;
		case 2: iBotCount = 9;
		case 0, 1: iBotCount = 6;
...
//			new iHeartBeats = 0;
	...
	//								if (GetEntProp(iSpawnPointRef, Prop_Send, "m_iTeamNum") != TEAM_SURVIVORS)
				...
				if (iHp <= CVAR_PLAYER_GLOW_HEALTH || g_iPlayerBleeding[client] != 0 || g_iPlayerInfected[client] != 0 || (g_fLastMedicCall[client] != 0.0 && g_fLastMedicCall[client] >= g_fGameTime))
					SetEntProp(client, Prop_Send, "m_bGlowEnabled", 1);
				else if (g_iPointFlagOwner != client && g_iMedicPlayer != client && g_iPlayerBleeding[client] == 0 && g_iPlayerInfected[client] == 0 && (g_fLastMedicCall[client] == 0.0 || g_fLastMedicCall[client] <= g_fGameTime))
					SetEntProp(client, Prop_Send, "m_bGlowEnabled", 0);
				iPlayerAliveCount++;
...
public Action:ThinkTimer(Handle:timer)
{
	...
	if (g_iMedicPlayer != -1 && IsClientInGame(g_iMedicPlayer) && g_iSecurityAlive+g_iSecurityDead > 2)
	{
		if (g_fGameTime-g_fMedicLastHealTime >= 300.0)
		{
			if (!g_bMedicForceToChange)
			{
...
public Action:CheckObjectForShouting(Handle:timer, any:iobject)
{
	if (iobject > MaxClients && IsValidEntity(iobject) && g_iCPType[g_iCurrentControlPoint] == 0 && EntRefToEntIndex(g_iCPIndex[g_iCurrentControlPoint]) == iobject)
	{
		if (g_iOnFireBy > 0 && g_iOnFireBy <= MaxClients && IsClientInGame(g_iOnFireBy) && IsPlayerAlive(g_iOnFireBy))
		{
			...
			FakeClientCommand(g_iOnFireBy, "say RUN!   Weapon cache is on FIRE!");
		}
	}
	return Plugin_Continue;
}
...
public Action:Event_PlayerTeam(Event event, const String:name[], bool:dontBroadcast)
{
	...
	if (g_iMedicPlayer == client) g_iMedicPlayer = -1;
	if (!IsFakeClient(client))
	{
...
public Action:Event_PlayerPickSquad(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (GetClientTeam(client) == TEAM_SURVIVORS)
	{
		if (GetEventInt(event, "squad_slot") != 0)
			RemoveCustomFlags(client);
		else
			CreateCustomFlag(client);
	}
...
public Action:Event_FlagDrop(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (IsClientInGame(client) && g_iMedicPlayer != client)
		SetEntProp(client, Prop_Send, "m_bGlowEnabled", 0);
	if (iTeam == TEAM_ZOMBIES)
	{
		if (g_sFlagSoundLast[0] != '\0') StopSoundAll(g_sFlagSoundLast, SNDCHAN_STATIC);
...
//	if (GetClientTeam(client) > TEAM_SPECTATOR)
	if (GetClientTeam(client) == TEAM_SURVIVORS)
	{
		...
		if (g_bMedicForceToChange && g_iMedicPlayer == client)
		{
			g_iMedicPlayer = -1;
...
public Action:Event_RoundFreezeEnd(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	g_iNextSpawnPointsIndex = -1;
	g_fMedicLastHealTime = GetGameTime();
	g_fThunderSound = g_fMedicLastHealTime+GetRandomFloat(5.0, 60.0);
//	SetWeaponCacheModel(-1, false);
...
stock bool:IsLeader(client)
{
	...
	if (GetPlayerSquadSlot(client, g_iPlayerManager, g_iOffsetSquadSlot) == 0)
		return true;
	return false;
}
...
{
	if (!IsClientInGame(client)) return false;
	if (GetPlayerSquad(client, g_iPlayerManager, g_iOffsetSquad) == 0 && GetPlayerSquadSlot(client, g_iPlayerManager, g_iOffsetSquadSlot) == 0)
		return true;
	return false;
}
...
{
	if (!IsClientInGame(client)) return false;
	if (GetPlayerSquad(client, g_iPlayerManager, g_iOffsetSquad) == 1 && GetPlayerSquadSlot(client, g_iPlayerManager, g_iOffsetSquadSlot) == 0)
		return true;
	return false;
}

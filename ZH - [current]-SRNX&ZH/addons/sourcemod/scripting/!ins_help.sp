...
stock ShowHelpMenu(client, page = 0)
{
	...
	if (page == 0)
	{
		...
		{
		Format(textformat, sizeof(textformat), "#WN|STF-Lua 僵尸暴动 - 游戏指南\n ");
			SetPanelTitle(PopupPanel,textformat);
			Format(textformat, sizeof(textformat), "[Steam]  \n[Discord]  \n ");
			DrawPanelText(PopupPanel, textformat);

			DrawPanelItem(PopupPanel, "游戏模式");				// 1
			DrawPanelItem(PopupPanel, "玩家装备");		// 2
//			DrawPanelItem(PopupPanel, "Zombie Classes");		// 3
			DrawPanelItem(PopupPanel, "", ITEMDRAW_NOTEXT);			// 3
			DrawPanelText(PopupPanel, "3. 僵尸职业 (WIP)");	// 3 TEXT
			DrawPanelItem(PopupPanel, "聊天指令");			// 4
			DrawPanelItem(PopupPanel, "提示\n ");				// 5
		}
		else
		{
	...
	else if (page == 1 || page >= 10 && page <= 19)		// Gamemode
	{
		if (page == 1)
		{
			if (StrContains(language, "korea", false) == -1)
			{
				...
				DrawPanelItem(PopupPanel, "情报获取");			// 11
				DrawPanelItem(PopupPanel, "隐藏的武器储备");	// 12
				DrawPanelItem(PopupPanel, "撤离直升机\n ");		// 13
			}
			else
			{
				Format(textformat, sizeof(textformat), "#Lua Zombie Horde\n[게임모드]\n ");
		...
		else if (page == 11)
		{
			if (StrContains(language, "korea", false) == -1)
			{
				Format(textformat, sizeof(textformat), "#WN|STF-Lua 僵尸暴动\n[游戏模式]  获取情报\n ");
				SetPanelTitle(PopupPanel,textformat);

				DrawPanelItem(PopupPanel, "武器储备");
				DrawPanelText(PopupPanel, "在获取情报之前武器储备将隐藏\n ");
				DrawPanelText(PopupPanel, "旗帜将会在敌人身上显示并带光圈");
				DrawPanelText(PopupPanel, "杀死旗帜携带者然后捡起旗帜");
				DrawPanelText(PopupPanel, "跟随蓝色的线路返回基地\n ");
			}
			else
			{
				Format(textformat, sizeof(textformat), "#Lua Zombie Horde\n[게임모드]  고가치 정보 회수\n ");
		...
		else if (page == 12)
		{
			if (StrContains(language, "korea", false) == -1)
			{
				...
				DrawPanelItem(PopupPanel, "武器储备");
				DrawPanelText(PopupPanel, "在找到携带者前武器储备将隐藏\n ");
				DrawPanelText(PopupPanel, "敌人可以携带武器储备四处移动");
				DrawPanelText(PopupPanel, "杀死敌人以找到武器储备携带者");
				DrawPanelText(PopupPanel, "有一定几率武器储备带有自爆炸弹\n ");
			}
			else
			{
				Format(textformat, sizeof(textformat), "#Lua Zombie Horde\n[게임모드]  숨겨진 무기 은닉처\n ");
